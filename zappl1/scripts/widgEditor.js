...
function widgToolbar(theEditor)
{
	...
	for (var i = 0; i < widgToolbarItems.length; i++)
	{
		switch (widgToolbarItems[i])
		{
			...
			case "video":
				this.addButton(this.theList.id + "Buttonvideo", "widgButtonvideo", "Insert Video", "video");
...
function widgToolbarAction()
{
	...
	switch (this.action)
	{
		...
		case "image":
			...
			case "video":
			var theImage = prompt("Enter the location for this video:", "");
			
			if (theImage != null && theImage != "")
			{
				var theAlt = prompt("Enter the alternate text for this video:", "");
				var theSelection = null;
				var theRange = null;
				...
				if (theIframe.contentWindow.document.selection)
				{
					...
					theRange = theSelection.createRange();
					theRange.collapse(false);
					theRange.pasteHTML("<video alt=\"" + theAlt + "\" src=\"" + theVideo + "\" />");
